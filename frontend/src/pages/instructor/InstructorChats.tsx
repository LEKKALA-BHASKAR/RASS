...
import { Send, MessageCircle, ChevronDown, BookOpen } from "lucide-react";
...
const InstructorChats: React.FC = () => {
  ...
  useEffect(() => {
    // Filter chats when selected course changes
    if (selectedCourse) {
      setFilteredChats(
        chats.filter(
          (chat) =>
            chat.course?._id === selectedCourse._id ||
            chat.course === selectedCourse._id
        )
      );
    } else {
      setFilteredChats(chats);
    }
  }, [selectedCourse, chats]);
  ...
  const loadChats = async (shouldScroll: boolean = true) => {
    try {
      ...
      // refresh selectedChat content if it exists
      if (selectedChat) {
        const updated = groupedChats.find((c: any) => c._id === selectedChat._id);
    ...
    <div>
      ...
      <div className="max-w-7xl mx-auto p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <motion.div
          ...
          className="border rounded-2xl p-5 bg-white shadow-xl relative"
        >
          ...
          {/* Fancy Course Selector */}
          <div className="relative mb-5">
            <button
              onClick={() => setDropdownOpen(!dropdownOpen)}
              className="w-full flex items-center justify-between bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 text-gray-800 rounded-xl px-4 py-2 font-medium shadow-sm hover:shadow-md transition"
            >
            ...
            <AnimatePresence>
              {dropdownOpen && (
                <motion.div
                  ...
                  className="absolute mt-2 w-full bg-white rounded-xl shadow-xl border border-gray-100 z-10 overflow-hidden"
                >
                  {courses.map((course) => (
                    <button
                      key={course._id}
          ...
          {filteredChats.length === 0 && (
            <p className="text-gray-500 text-sm">No chats for this course.</p>
          )}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50">
                <AnimatePresence>
                  {selectedChat.messages.map((msg: any, idx: number) => {
                    const isStudent =
                      msg.sender._id === selectedChat.student._id;
                    return (
                      <motion.div
                        key={idx}
                        initial={{ opacity: 0, y: 10 }}
