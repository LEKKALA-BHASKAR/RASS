...
const InstructorDiscussions: React.FC = () => {
  ...
  try {
    ...
    // 🔥 Reload discussions
    await loadDiscussions(selectedCourse._id);

    // 🔥 Auto expand the new post
    if (res.data?._id) {
      setExpanded(new Set([res.data._id]));
    }
  } catch (err) {
  ...
  try {
    await forumAPI.addReply(postId, replying.content);
    ...
    // 🔥 Keep the replied post expanded
                      ...
                      {expanded.has(post._id) && (
                        ...
                        >
                          ...
                          {replying?.postId === post._id ? (
                            ...
                            <button
                              onClick={() =>
                                setReplying({ postId: post._id, content: "" })
                              }
