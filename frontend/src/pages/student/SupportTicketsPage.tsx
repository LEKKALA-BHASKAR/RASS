...
interface SupportTicketsProps {
  courseId?: string; // optional for embedding in course player
  embedded?: boolean; // hide Navbar/Footer if true
}

const SupportTickets: React.FC<SupportTicketsProps> = ({ courseId, embedded = false }) => {
  ...
  const fetchTickets = async () => {
    try {
      ...
      // 🔹 Filter by course if embedded inside CoursePlayer
      if (courseId) {
        data = data.filter(
          (ticket: any) => ticket.course?._id === courseId || ticket.course === courseId
        );
      }
  ...
  const handleCreateTicket = async (e: React.FormEvent) => {
    ...
    try {
      const ticketPayload = courseId ? { ...newTicket, course: courseId } : newTicket;
      const response = await supportTicketAPI.createTicket(ticketPayload);
